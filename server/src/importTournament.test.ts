...
async function canonicalId(db: Pool, normalized: string): Promise<number> {
...
test('imports a tournament and reports accurate counts', async () => {
  const db = await makeTestDb();
  const summary = await importTournament(db, readFixture('sample.xml'));
  ...
  const opens = (
    await db.query<{ id: number; date: string | null }>(
      'SELECT id, date FROM events WHERE id IN (1,2) ORDER BY id',
    )
  ).rows;
  ...
  // jigo row
  const jigo = (
    await db.query<{ w: number | null }>(
      "SELECT winner_event_player_id AS w FROM games WHERE result_type = 'draw'",
    )
  ).rows[0];
  ...
  // by-default row
  const bydef = (
    await db.query<{ is_game: number }>(
      "SELECT is_game FROM games WHERE result_type = 'forfeit'",
    )
  ).rows[0];
  assert.equal(bydef.is_game, 0);
});

test('re-importing the identical file is rejected as a duplicate', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));
  let caught: unknown;
  try {
    await importTournament(db, readFixture('sample.xml'));
  } catch (e) {
  ...
  const count = (await db.query<{ n: number }>('SELECT COUNT(*)::int AS n FROM events')).rows[0]
    .n;
  assert.equal(count, 3);
});

test('a shared player across two files links to one canonical player', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));
  const second = await importTournament(db, readFixture('sample2.xml'));

  assert.equal(second.playersMatched, 1); // "JOHN SMITH" matches "John Smith"
  assert.equal(second.playersCreated, 1); // "Carol King" is new

  const johnId = await canonicalId(db, 'john smith');
  const links = (
    await db.query<{ n: number }>(
      'SELECT COUNT(*)::int AS n FROM event_players WHERE player_id = $1',
      [johnId],
    )
  ).rows[0];
...
test('player detail splits opponents into losing / even / winning with W–L', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));
  await importTournament(db, readFixture('sample2.xml'));

  const johnId = await canonicalId(db, 'john smith');
  const detail = (await getPlayerDetail(db, johnId))!;

  assert.equal(detail.player.name, 'John Smith');
  assert.equal(detail.events.length, 2);

  const winning = detail.winning.find((r) => r.opponentName === 'Jane Doe');
  assert.deepEqual({ w: winning?.wins, l: winning?.losses }, { w: 1, l: 0 });
...
test('history is reverse-chronological, spans events, and includes non-games', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));
  await importTournament(db, readFixture('sample2.xml'));

  const johnId = await canonicalId(db, 'john smith');
  const page = await getPlayerHistory(db, johnId, 1);
...
test('matchups requires a filter and returns canonical names', async () => {
  const db = await makeTestDb();
  const { eventId } = await importTournament(db, readFixture('sample.xml'));

  await assert.rejects(() => getMatchups(db, {}));

  const rows = await getMatchups(db, { event: eventId });
...
test('remapEventPlayer repoints a mis-matched player without re-import', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));

  const bob = (
    await db.query<{ id: number; player_id: number }>(
      "SELECT id, player_id FROM event_players WHERE og_key = 'JONESBOB'",
    )
  ).rows[0];

  const res = await remapEventPlayer(db, bob.id, { newName: 'Robert Jones' });
...
test('listPlayers / listEvents surface counts', async () => {
  const db = await makeTestDb();
  await importTournament(db, readFixture('sample.xml'));
  ...
  const events = (await listEvents(db)).filter((e) => e.id > 2);
