...
async function canonicalId(db: Queryable, normalized: string): Promise<number> {
  return (
    (
...
test('imports a tournament and reports accurate counts', async (t) => {
  ...
  const opens = (
    await db.query('SELECT id, date FROM events WHERE id IN (1,2) ORDER BY id')
  ).rows as { id: number; date: string | null }[];
  ...
  ).rows[0] as { w: number | null };
  assert.equal(jigo.w, null);

  // by-default row
  const bydef = (
    await db.query("SELECT is_game FROM games WHERE result_type = 'forfeit'")
  ).rows[0] as { is_game: number };
  assert.equal(bydef.is_game, 0);
});

test('re-importing the identical file is rejected as a duplicate', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
  ...
  assert.equal(
    ((await db.query('SELECT COUNT(*)::int AS n FROM events')).rows[0] as { n: number })
      .n,
    3,
  );
});

test('a shared player across two files links to one canonical player', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
...
test('player detail splits opponents into losing / even / winning with W–L', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
...
test('history is reverse-chronological, spans events, and includes non-games', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
...
test('matchups requires a filter and returns canonical names', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
...
test('remapEventPlayer repoints a mis-matched player without re-import', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
  ...
  ).rows[0] as { id: number; player_id: number };
...
test('listPlayers / listEvents surface counts', async (t) => {
  const { db, cleanup } = await makeTestDb();
  t.after(cleanup);
