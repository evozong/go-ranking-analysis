...
test('resolveCanonicalPlayer creates on first sight, matches case-insensitively after', async () => {
  ...
  const count = (await db.query('SELECT COUNT(*)::int AS n FROM players')).rows[0].n;
  assert.equal(count, 2);
});

test('the first-seen display name is preserved on the canonical row', async () => {
  const db = await makeTestDb();
